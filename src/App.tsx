...
type ProfileEditFocus = "personality" | "primaryVet" | null;
...
function textStatusClassForTone(tone: "green" | "amber" | "coral" | "gray") {
  if (tone === "green") return "status-text status-text-green";
  if (tone === "amber") return "status-text status-text-amber";
  if (tone === "coral") return "status-text status-text-coral";
  return "status-text status-text-gray";
}
...
export default function App() {
  ...
  const [profileEditIntent, setProfileEditIntent] = useState<ProfileEditFocus>(null);
    ...
    <main className="app-root">
      ...
      {tab === "profile" && (
        <ProfileScreen
          ...
          onEditIntentConsumed={() => setProfileEditIntent(null)}
    ...
    <section className="screen">
      ...
      {activeCareTab === "Summary" ? (
        <MedicalSummaryPanel
          ...
          onEditPrimaryVet={onOpenPrimaryVetEdit}
                ...
                <div className="care-copy">
                  {planStatus ? (
                    <div className="badge-row">
                      <span className={textStatusClassForTone(planStatus === "Active" ? "green" : planStatus === "Upcoming" ? "amber" : planStatus === "Ended" ? "gray" : "coral")}>{planStatus}</span>
                    </div>
                  ) : (
                    <span className={textStatusClassForTone(careStatus(record) === "OK" ? "green" : careStatus(record) === "Overdue" ? "coral" : "amber")}>
                      {careStatus(record)}
                    </span>
                  )}
                  <h2>{record.title}</h2>
...
function CareHistoryPanel({
  ...
  if (activeTab === "Docs" || activeTab === "Summary" || activeTab === "Meds" || activeTab === "Vaccines" || activeTab === "Vet visits") return null;
    ...
    <section className="medical-summary-panel">
      <section className="card medical-summary-card">
        <div className="medical-summary-head">
          <p className="label no-margin">Primary vet</p>
          <button className="link-btn" type="button" onClick={onEditPrimaryVet}>
            {hasPrimaryVet ? "Edit" : "Add vet"}
          </button>
        </div>
        {hasPrimaryVet ? (
          <div className="detail-grid">
            {profile.primaryVetClinic ? <div className="detail-row"><span>Clinic</span><strong>{profile.primaryVetClinic}</strong></div> : null}
            {profile.primaryVetName ? <div className="detail-row"><span>Vet</span><strong>{profile.primaryVetName}</strong></div> : null}
            {profile.primaryVetPhone ? <div className="detail-row"><span>Phone</span><strong>{profile.primaryVetPhone}</strong></div> : null}
            {profile.primaryVetEmail ? <div className="detail-row"><span>Email</span><strong>{profile.primaryVetEmail}</strong></div> : null}
            {profile.primaryVetAddress ? <div className="detail-row"><span>Address</span><strong>{profile.primaryVetAddress}</strong></div> : null}
          </div>
        ) : (
          <p>No primary vet saved yet.</p>
        )}
      </section>
      ...
      <section className="card medical-summary-card">
        <div className="medical-summary-head">
          ...
          <span className="summary-count">{summary.activeMedications.length || 0}</span>
        </div>
...
function ProfileScreen({
  ...
  const [editFocus, setEditFocus] = useState<ProfileEditFocus>(null);
  const [climateOpen, setClimateOpen] = useState(false);
  const [locationStatus, setLocationStatus] = useState("");
  useEffect(() => {
    if (!initialEditFocus) return;
    setEditFocus(initialEditFocus);
    setEditing(true);
    onEditIntentConsumed();
  }, [initialEditFocus, onEditIntentConsumed]);
  ...
  return (
    <section className="screen profile-screen">
      ...
      <section className="card settings-card">
        ...
        />
        <div className={climateOpen ? "coach-location-settings open" : "coach-location-settings"}>
          <button
            className="setting-row climate-care-toggle"
            type="button"
...
function ProfileEditSheet({
  ...
  const primaryVetRef = useRef<HTMLInputElement | null>(null);

  useEffect(() => {
    if (initialFocus === "personality") personalityRef.current?.focus();
    if (initialFocus === "primaryVet") primaryVetRef.current?.focus();
  }, [initialFocus]);
  ...
  return (
    <Sheet title="Edit profile" onClose={onClose}>
      ...
      <section className="card profile-preview">
        <p className="label no-margin">Primary vet</p>
        <div className="form-two">
          <Field label="Clinic">
            <input
              ref={primaryVetRef}
              className="input"
              value={draft.primaryVetClinic || ""}
              onChange={(event) => setDraft((current) => ({ ...current, primaryVetClinic: event.target.value }))}
            />
          </Field>
          <Field label="Vet name">
            <input
              className="input"
              value={draft.primaryVetName || ""}
              onChange={(event) => setDraft((current) => ({ ...current, primaryVetName: event.target.value }))}
            />
          </Field>
        </div>
        <div className="form-two">
          <Field label="Phone">
            <input
              className="input"
              value={draft.primaryVetPhone || ""}
              onChange={(event) => setDraft((current) => ({ ...current, primaryVetPhone: event.target.value }))}
            />
          </Field>
          <Field label="Email">
            <input
              className="input"
              value={draft.primaryVetEmail || ""}
              onChange={(event) => setDraft((current) => ({ ...current, primaryVetEmail: event.target.value }))}
            />
          </Field>
        </div>
        <Field label="Address">
          <input
            className="input"
            value={draft.primaryVetAddress || ""}
            onChange={(event) => setDraft((current) => ({ ...current, primaryVetAddress: event.target.value }))}
          />
        </Field>
      </section>
